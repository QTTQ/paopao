...
var (
	downLoadDirPath = config1.Get("qiniu_config > url").(string)
	config1         = parseConfig.New("config/conf.json")
	accessKey       = config1.Get("qiniu_config > accessKey").(string)
	secretKey       = config1.Get("qiniu_config > secretKey").(string)
	bucket          = config1.Get("qiniu_config > bucket").(string)
	videoDomain     = config1.Get("qiniu_config > url").(string)
...
func downloadQiNiu(localFile string) string {
	domain := videoDomain
	key := localFile
...
func CreatArticle(c *gin.Context) {
	...
	if form != nil {
		...
		for _, file := range files {
			paths += downLoadDirPath + DST + file.Filename + ","
